...
function deslizarParaDireita() {
    ...
    if (leftAvatar < posicaoEsquerdaFinal) {
        let novoLeft = Math.min(leftAvatar + parseInt(txtPixel.value), posicaoEsquerdaFinal);
        imgAvatar.style.transition = "left 0.5s ease-in-out";
        imgAvatar.style.left = novoLeft + "px";

        setTimeout(() => {
            imgAvatar.style.transition = "";
        }, 200);
    } else {
